#a list name "question stores all the 15 question of KBC game"
...
#"first_options" list contains all the first options of all the question and so the others list too like second_options,third_options and fourth_options.
...
#another list which contains all the other list that are mentioned above
all_options = [first_options , second_options , third_options , fourth_options]
correct_answers = 0
ans_key = [3 , 1 , 2 , 1 , 1 , 3 , 0 , 1 , 2 , 3 , 0 , 3 , 1 , 0, 1] # this list contains all the correct of answers of all 15 questions respectively as a index number
...
while i < len(questions):
      print questions[i]  #printing a question
      print first_options[i]
      print second_options[i]
      print third_options[i]
      print fourth_options[i]
      ans = int(raw_input("Enter correct answers in the form 0,1,2,3 :- ")) #asking the user for the answers among the options
      answers_list.append(ans) # inserting our answers into another list
      if ans_key[i] == ans: # if our ansers is correct then it will do the following things mentioned inside the if statements
      	 correct_answers = correct_answers + 1
         print "Congrats! Aapka answer sahi " , winer_price[i] , "winer price aapko mila hai" , correct_answers , "answer sahi hey aapka"
         sum = winer_price[i] 
         
         if i == 4:
          	 print "Congrats! Aapka  phela padaav pura ho gaya hai."
         elif i == 9:
          		print "Congrats! Aapka doosra padaav pura ho gaya hai."
         elif i == 14:
          		print "Congrats! Aap ek crore rupye jeet gaye hai." 
      else:# if user's answer is not correct then loop will break and game will be stoped with the message that how much cash prize does a player has won .
